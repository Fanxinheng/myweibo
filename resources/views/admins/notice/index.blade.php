...
@endif
    ...
    <thead>
        <tr role="row">
            ...
            <th class="" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 255px;">内容</th>
        ...
        <tbody role="alert" aria-live="polite" aria-relevant="all" style="overflow:hidden;">
             @foreach($res as $k => $v)
            <tr class="@if ( $v->id % 2 == 0 ) odd @else even @endif" align="center" id="notice{{$v->id}}">
                <td class="">{{$v->id}}</td>
                <td class="">{{$v->title}}</td>
